...
int main(int argc, char* argv[]){
	...
	in.close();
	printf("\nMostrar las cinco primeras filas de la lectura del fichero\n\n");
	for (int i = 0; i < 5; i++){
		for (int j = 0; j < numeroDeColumnas; j++){
			printf("%f ", x[i * numeroDeColumnas + j]);
		}
		printf("\n");
	}
	//Lectura del fichero
	//Calcular media para cada columna;
	printf("\nValores del vector centroide\n\n");
	...
	printf("\n\nNormalizacion de las cinco primeras filas\n\n");
	for (int i = 0; i < 5; i++){
		for (int j = 0; j < numeroDeColumnas; j++){
			printf("%f ", x[i*numeroDeColumnas + j]);
		}
	...
	// Calcular Y = X*V
	printf("\nMostrar las cinco primeras filas de Y = X*V\n\n");
	...
	for (int i = 0; i < 5; i++){
		for (int j = 0; j < numeroDeColumnas; j++){
			printf("%f ", y[i * numeroDeColumnas + j]);
		}
		printf("\n");
	}
	...
	fclose(out);
	printf("\nAutores:Aitor Saavedra Hernandez, Laura Del Pino Diaz");
	std::getchar();
	return 0;
}
